...
// Studio-wide Google rating shown in the summary panel + the "leave a
// review" button target. GOOGLE_REVIEW_URL is the studio's public
// "write a review" link (Google Business Profile > Share review form).
const GOOGLE_RATING = "4.9";
const GOOGLE_REVIEW_COUNT = 117;
const GOOGLE_REVIEW_URL = null;

// Real reviews copied from the studio's Google Business listing — edit this
// list directly to add/update/remove reviews (no admin panel involved).
const GOOGLE_REVIEWS = [
  ...
  },
  {
    ...
    timeAgo: "3 years ago",
    rating: 5,
    text: "Amazing dance studio. All the instructors are extremely talented and helpful to people of all ages and skill. Just Love it here!",
    avatarUrl: null,
  },
];
...
function GoogleReviewCard({ review }) {
  return (
    <div className="glass w-[280px] shrink-0 snap-start rounded-2xl p-5">
      ...
      <p className="mt-3 max-h-24 overflow-y-auto text-sm text-foreground/90">
        {review.text}
      </p>
    </div>
  );
...
// Google reviews widget + achievements strip. Rating/review data is static —
// copied directly from the studio's Google Business listing (edit the
// constants above), no admin panel or live API involved.
export default function Reviews() {
  ...
  function scrollBy(dir) {
    scrollToIndex(Math.min(Math.max(activeIndex + dir, 0), GOOGLE_REVIEWS.length - 1));
  }
      ...
      <div className="glass flex flex-col gap-8 rounded-3xl p-6 md:flex-row md:items-center md:p-8">
        <div className="flex flex-col gap-3 md:w-56 md:shrink-0 md:border-r md:border-border md:pr-8">
          ...
          <div className="flex items-center gap-2">
            <span className="text-2xl font-bold text-brand-lime">{GOOGLE_RATING}</span>
            <Stars rating={5} />
          </div>
        ...
        <div className="relative flex-1">
          ...
          <button
            ...
            onClick={() => scrollBy(1)}
            disabled={activeIndex >= GOOGLE_REVIEWS.length - 1}
