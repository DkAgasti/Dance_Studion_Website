...
export const admissionSchema = z.object({
  ...
  // Plan Selection — the real batch (schedule + live price) for the chosen
  // class. Optional at submission since a class may not have batches set up
  // yet; admin still picks the actual batch when approving the admission.
...
// Admin status update (PATCH /api/admissions/[id]). Approving requires a
// batchId so the applicant can be enrolled as a Student in one step.
export const admissionStatusUpdateSchema = z
  .object({
    status: z.enum(["PENDING", "APPROVED", "REJECTED"], { error: "Invalid status." }),
    batchId: z.string().optional(),
  })
  .refine((data) => data.status !== "APPROVED" || !!data.batchId, {
    message: "Please select a batch to approve and enroll this applicant.",
    path: ["batchId"],
  });
