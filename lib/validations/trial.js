...
// Admin status update (PATCH /api/trial-bookings/[id]). Marking a booking
// CONVERTED also creates an Admission, so a real classId is required then.
export const trialStatusUpdateSchema = z
  .object({
  ...
  .refine((data) => data.status !== "CONVERTED" || !!data.classId, {
    message: "Please select a class to create the admission.",
    path: ["classId"],
  });
